...
async def create_run(
    ...
    try:
        sessions, atr_by_date = await load_sessions(db, req.params)
        if not sessions:
            raise ValueError("no 1m bars found for this instrument/date range")
        results = run_backtest(sessions, req.params, atr_by_date=atr_by_date)
    ...
    logger.info(
        ...
        run_id, req.params.instrument, len(results), runtime_ms,
    )
    return RunResponse(
        run_id=run_id,
        runtime_ms=runtime_ms,
